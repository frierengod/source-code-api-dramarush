<?php
/**
 * Contoh Script Bridge API Dramarush via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Dramarush di sistem Bridge
function callDramarushAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/dramarush/api/v1";

// Contoh 1: Fitur Cari Film Dramarush
echo "\n=====================[ 1. Pencarian Dramarush ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callDramarushAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callDramarushAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callDramarushAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/dramarush ◀◀◀\n\n";
